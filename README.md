...
- To make all the structure of the robot, it took a lot of time and investigation, we decided to build our robot totally by our selves, the robot's body was make in [OnShape](https://www.onshape.com/en/) Platform , all the design of the robot and all the models and pieces can be found here in `models`, it is compound by 17 parts assembled together. We have passed by a lot of prototypes, at first heavier and bigger, but for turkiye prototype, to make it more light, more smaller, more efficient, we are looking to make the things simple, to make the most efficient autonomous vehicle, we decided to use [Arduino Nano](https://store.arduino.cc/products/arduino-nano) , because it's smaller and has everything that we need to control our robot, we have use a various types of Arduino nano, Like the [Arduino Nano ESP32](https://store.arduino.cc/products/nano-esp32), [Arduino Nano Every](https://store.arduino.cc/products/arduino-nano-every), [Arduino Nano Every](https://store.arduino.cc/products/arduino-nano-every). The reason of why we have used various types of arduino nano, is because every one of them give different capacities, and sometimes we need different capacities in our robot. To understand the making and the programming of the robot please check all the parts of this `README.md`.
...
This circuit diagram represents the connections of a robotic system powered by a battery pack and controlled by an Arduino Nano. It integrates three ultrasonic sensors for obstacle detection, a servo motor for steering, and a DC motor for torque and speed via a TB6612 motor driver module. The MPU6050 sensor provides orientation data (gyroscope and accelerometer), likely used for navigation adjustments. A Pixy Cam is included for visual object tracking. Connections include multiple power and ground lines to ensure stable operation, while the battery supplies energy to the motors and sensors. UTP wires link the Arduino to the motor driver, servo, and sensors, ensuring precise control and data flow.
...
int calcularAnguloObjetivoI(int curva) {
    if (curva % 4 == 1) return -75;     // Curvse 1, 5, 9, ...
...
int calcularAnguloObjetivoDC(int curva) {
    if (curva % 4 == 1) return 90;     // Curves 1, 5, 9, ...
    else if (curva % 4 == 2) return 180; // Curves 2, 6, 10, ...
    else if (curva % 4 == 3) return -90;   // Curves 3, 7, 11, ...
    else return 0;                      // Curves 4, 8, 12, ...
}
